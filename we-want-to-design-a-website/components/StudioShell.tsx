    ...
    const draw = (now: number) => {
      ...
      for (let y = 14; y < height; y += spacing) {
        for (let x = 12; x < width; x += spacing) {
          const wave =
            Math.sin(x * 0.011 + time * 2.2) * 12 +
            Math.cos(y * 0.014 - time * 1.5) * 8;
          ...
          const px =
            x + (distance ? (dx / distance) * push : 0);

          const py =
            y + wave + (distance ? (dy / distance) * push : 0);

          const radius = 0.7 + influence * 2.6;

          const hue =
            188 +
            Math.sin(x * 0.006 + time) * 30 +
            influence * 40;
...
function KineticCursor() {
  ...
  return (
    <motion.div
      style={{ x: sx, y: sy }}
      className={cn("kinetic-cursor", hover && "is-hovering")}
    >
      <span>✦</span>
    </motion.div>
  );
...
function useSound() {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("logicflow-sound", String(enabled));
    } catch {}
  }, [enabled]);
...
export default function StudioShell({
  ...
  const [dark, setDark] = useState<boolean>(() => {
    ...
    const attr =
      document.documentElement.dataset.logicflowTheme;
    ...
    try {
      return (
        localStorage.getItem("logicflow-theme") !== "light"
      );
    } catch {
      return true;
    }
  ...
  useEffect(() => {
    ...
    try {
      ...
      document.body.style.backgroundColor =
        theme === "dark" ? "#080c14" : "#fff9f3";

      window.dispatchEvent(
        new CustomEvent("logicflow-theme-change", {
          detail: theme,
        })
      );
    } catch {}
  ...
  return (
    <main
      className={cn(
        "colorful-ui custom-cursor relative min-h-screen overflow-hidden bg-paper text-ink transition-colors duration-300",
        dark ? "dark" : "light"
      )}
    >
      ...
      <header className="site-nav mx-auto flex max-w-7xl items-center justify-between px-6 py-5 lg:px-8">
        <Link
          ...
          onClick={() => sound.click(true)}
          aria-label="Go to LogicFlow home"
        >
          {/* Home button */}
          <div className="grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br from-mango via-coral to-pink text-white shadow-lg shadow-pink/25 transition-transform duration-200 hover:scale-105">
            <svg
      ...
      <footer className="border-t border-slate-200 bg-white/80">
        <div className="mx-auto flex max-w-7xl flex-col justify-between gap-3 px-6 py-6 text-sm text-slate-500 sm:flex-row lg:px-8">
          <p>
            LogicFlow · Build, simplify, probe, and
            understand digital logic.
          </p>

          <p>
            Your UI · Reference feature engine.
          </p>
        </div>
      </footer>
    </main>
